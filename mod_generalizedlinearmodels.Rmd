...
In this section, we will be using the packages `r cran_link("catdata")`, `r cran_link("MASS")`, `r cran_link("AER")`, `r cran_link("performance")`, `r cran_link("faraway")` , `r cran_link("LogisticDx")`, and `r cran_link("car")`. We can load these packages in R after installing them by the following:
...
```
which provides us with estimates of the odds ratio (OR), andwhere lowOR and upOR are the lower and upper bounds to the 95% CI. The two lines are for two models: one with an intercept and without, though in this case the OR and CI's are the same. We can also find the CI manually by finding the estimated standard error of the $\widehat{\beta_2} + \widehat{\beta_5}$ manually.
...
```
Our $p$-value is very small (<0.000) and indicates that we cannot assume the outcome is normal for this data set. As such, we can turn to the Gamma GLM to model this outcome and see if the rent is related to any other covariates in the data set. 
