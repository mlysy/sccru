...
## Gamma Distributed (Skewed) Outcomes

For continuous data that is not not normally distributed, a GLM using the gamma distribution can be used to model skewed outcomes. 
...
$y$ for a one unit increase in $x_j$, when the other coefficients are equal to zero. 
...
```{r glm-gamma-hist, fig.cap = "Histogram of Resistivity"}
# create a histogram of the outcome
hist(wafer$resist, breaks = 5)
```

```{r glm-gamma-boxplot, fig.cap = "Boxplot of Resistivity"}
boxplot(wafer$resist)
```
Both of the plots in Figures \@ref(fig:glm-gamma-hist) and \@ref(fig:glm-gamma-boxplot) indicate that the outcome data is skewed and non-normal. As the outcome data is non-normal, it is inappropriate to use the normal distribution for modelling. However, we can model such outcomes using the gamma distribution.
