...
In this chapter, we will be using the packages `r cran_link("catdata")`, `r cran_link("MASS")`, `r cran_link("AER")`, `r cran_link("performance")`, `r cran_link("faraway")` , `r cran_link("LogisticDx")`, and `r cran_link("car")`. We can load these packages in R after installing them by the following:
...
- $\beta_1$: For a one unit increase in the average age, we estimate the log odds of menarche to be 1.632 times higher (the odds ratio is estimated to be $\exp(1.632)$ = 5.114 times higher). Confidence intervals can be calculated as in Chapter \@ref(glm-logreg).
...
### Example {#logreg-poisson-example}
...
#### Model Fitting {#logreg-poisson-example-mf}
...
```
We again have a small $p$-value and reject the null hypothesis that the simpler model fit is adequate. That is, the main-effects onlyull model fits better between these two models. 
...
#### Model Diagnostics {#logreg-poisson-example-md}
...
\end{aligned}.\
$$
Under the log link, we have a multiplicative model. We interpret $\exp(\beta_0)$ as the expected value of $y$ when all covariates are equal to zero.  We interpret $\exp(\beta_j$) as the expected multiplicative increase of
$y$ for a one unit increase in $x_j$, when the other covariates are equal to zero. 
...
```{r glm-gamma-ex-hist, fig.cap = "Histogram showing skewed outcome"}
# create a histogram of the outcome
hist(wafer$resist, main = "Histogram of Resistivity", 
     xlab = "Resistivity")
```

```{r glm-gamma-ex-boxplot, fig.cap = "Boxplot showing skewed outcome"}
boxplot(wafer$resist, main = "Boxplot of Resistivity", 
        ylab = "Resistivity")
```
Both of the plots in Figures \@ref(fig:glm-gamma-ex-hist) and \@ref(fig:glm-gamma-ex-boxplot) indicate that the outcome data is skewed and non-normal. As the outcome data is non-normal, it is inappropriate to use the normal distribution for modelling. However, we can model such outcomes using the gamma distribution.

#### Model Fitting {#glm-gamma-example-mf}

We fit a gamma GLM with a log link, as described in the previous section. To do so in R, we perform the following:
```{r glm-gamma-ex-fit1}
fit1_wafer <- glm(resist ~ x1 + x2 + x3 + x4, data = wafer,
                  family = Gamma(link = "log"))
summary(fit1_wafer)
```
We see from the output of the main-effects only model that $x_1$, $x_2$, and $x_3$ are significantly associated with the resistivity. Similar to other model building procedures, we should consider interaction effects in our model. Although $x_4$ is not statistically significant, interactions involving $x_4$ may be significant so we should use hypothesis testing to formally investigate which covariates and interactions to include in the model.

We can look at adding each 2-way interaction term one by one, or start by comparing the main-effects only model to a model including all 2-way interactions. We fit the model using all two-way interaction effects and perform a hypothesis test of $H_0:$ the simpler model (main-effects only) is adequate compared to the fuller model (interaction model) using an LRT as:
```{r glm-gamma-ex-fit2}
# fit interaction model
fit2_wafer <- glm(resist ~ x1*x2 + x1*x3 + x1*x4 + x2*x3 + x2*x4 + x3*x4, 
                  data = wafer,
                  family = Gamma(link = "log")) # fit interaction model

# perform the LRT
anova(fit1_wafer, fit2_wafer, test = "LRT")
```
With a small $p$-value, we reject the null hypothesis and conclude that we should continue  model building with the interaction model. Let's look at a summary of the model:
```{r glm-gamma-ex-fit2summary}
summary(fit2_wafer)
```
We notice that some of the two-way interactions are insignificant, and should consider models that do not include them. As such, we should compare the existing model with one that does not contain the the least statistically significant interaction term. In this case, we should fit a model without the $x_1$ and $x_4$ interaction and see if the fit is adequate.
```{r glm-gamma-ex-fit3}
# fit interaction model without x_1:x_4
fit3_wafer <- glm(resist ~ x1 + x2 + x3 + x4 + 
                    x1:x2 + x1:x3 + x2:x3 + x2:x4 + x3:x4, 
                  data = wafer,
                  family = Gamma(link = "log")) # fit interaction model

# perform the LRT, comparing to full interaction model
anova(fit2_wafer, fit3_wafer, test = "LRT")
```
We obtain a large $p$-value from the hypothesis test that the simpler model is adequate, so we conclude that we do not need the $x_1:x_4$ interaction term in the model. Let's look at the current model output:
```{r glm-gamma-ex-fit3summary}
summary(fit3_wafer)
```
We repeat the previous steps for the $x_1:x_2$ interaction term.
```{r glm-gamma-ex-fit4}
# fit interaction model without x_1:x_2
fit4_wafer <- glm(resist ~ x1 + x2 + x3 + x4 + 
                    + x1:x3 + x2:x3 + x2:x4 + x3:x4, 
                  data = wafer,
                  family = Gamma(link = "log")) # fit interaction model

# perform the LRT, comparing to full interaction model
anova(fit3_wafer, fit4_wafer, test = "LRT")
```
We again obtain a large $p$-value from the hypothesis test that the simpler model is adequate, so we conclude that we do not need the $x_1:x_2$ interaction term in the model. Let's look at the current model output:
```{r glm-gamma-ex-fit4summary}
summary(fit4_wafer)
```
We repeat the previous steps for the $x_2:x_4$ interaction term.
```{r glm-gamma-ex-fit5}
# fit interaction model without x2:x4 interaction
fit5_wafer <- glm(resist ~ x1 + x2 + x3 + x4 + 
                    + x1:x3 + x2:x3 + x3:x4, 
                  data = wafer,
                  family = Gamma(link = "log")) # fit interaction model

# perform the LRT, comparing to full interaction model
anova(fit4_wafer, fit5_wafer, test = "LRT")
```
We again obtain a large $p$-value from the hypothesis test that the simpler model is adequate, so we conclude that we do not need the $x_2:x_4$ interaction term in the model. Let's look at the current model output:
```{r glm-gamma-ex-fit5summary}
summary(fit5_wafer)
```
All of the interaction terms in this model are significant. Although $x_4$ is not individually significant, we keep it in the model as we do have one interaction term involving $x_4$. We select this model (`fit5_wafer`) to interpret.

#### Model Diagnostics {#glm-gamma-example-md}

Prior to interpreting our model, we need to assess the model fit. To do so, we can take a look at the residual plot to see if there are any large residuals that may indicate a poor model fit. 
```{r glm-gamma-ex-resid, fig.cap = "Residual plot for Gamma GLM"}
resid_wafer <- residuals.glm(fit1_wafer, "deviance")
plot(resid_wafer, ylim = c(-3, 3),
     main = "Deviance Residuals",
     ylab = "Residuals")
abline(h = -2, lty = 2) # add dotted lines at 2 and -2
abline(h = 2, lty = 2)
```
We see that we have very small residuals for all observations, indicating no lack-of fit. 

We can also check if there is multicollinearity present in the final model using the `vif()` function. However, we do not anticipate this to be an issue for this specific study as we are using a full factorial design where we independently manipulate the four covariates. 
```{r glm-gamma-ex-vif}
vif(fit5_wafer)
```
The VIF values are all considered low (< 5) and do not indicate issues with collinearity.

These diagnostics do not indicate a poor fit and thus the model we chose can be interpreted. 

#### Model Interpretation {#glm-gamma-example-interpretation}

We can write the chosen model as

$$
\log(y_i) = \beta_0 + \beta_1x_1 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_1x_3 + \beta_6x_2x_3 + \beta_7x_3x_4
$$
or equivalently 
$$
\begin{aligned}
y_i &= \exp(\beta_0 + \beta_1x_1 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_1x_3 + \beta_6x_2x_3 + \beta_7x_3x_4)\\
&=\exp(\beta_0)\exp(x_1)^{\beta_1}\exp(x_2)^{\beta_2}\exp(x_3)^{\beta_3}\exp(x_4)^{\beta_4}\exp(x_1x_3)^{\beta_5}\exp(x_2x_3)^{\beta_6}\exp(x_3x_4)^{\beta_7}.
\end{aligned}
$$

From this model, we interpret $\exp(\beta_0) = \exp(5.31195) = 202.745$ as the expected resitivity when all covariates are equal to "-". 

We interpret $\exp(\beta_1) = \exp(0.20029) = 1.222$ as the expected multiplicative increase of the resistivity when $x_1$ is "+", when the other covariates are "-". We similarly interpret the coefficients for $x_2$, $x_3$, and $x_4$, although we note the individual effect for $x_4$ is not statistically significant. For the interaction term $x_1:x_3$, we interpret $\exp(\beta_5) = \exp(-0.15549) = 0.856$ as the additional multiplicative increase in resistivity when both $x_1$ and $x_3$ are equal to "+", and all other covariates are equal to "-". We similarly interpret the other interaction terms. 
