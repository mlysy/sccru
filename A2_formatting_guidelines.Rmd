...
- Let's agree to use regular font for programming languages (e.g., R, Python, C++) and bold font for packages and libraries (e.g., **TMB**, **rstan**, **NumPy**, **Tensorflow**). Use `data` format for datasets, codes and arguments. For functions, use parentheses at the end, for example, `data()`. Use curly brackets `\bm{}` for LaTeX functions.
...
- Please see Appendix [B.2.3](#a1-citations) for how to handle citations.
...
- To make cross referencing consistent in bookdown, for all labels, including r chunks, figures, tables, sections, equations, etc., use the prefix of the corresponding file as the prefix of any label. For example, use lablel `#a2-general` for this section.  
...
- For multi-line equations with alignment, use `aligned` environment as in \@ref(eq:a2-mathex).
...
- Independent/IID/Normal: Please see \@ref(eq:a2-mathex).
