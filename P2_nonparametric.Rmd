...
- the data contains outliers. (This reason only applied to rank based methods, which are more resistant to outliers.)
...
|----|-----|-----|
| Independent      | Difference in Mean | [Wilcoxon rank-sum](#mwu) |
| Paired | Difference in Mean | [Wilcoxon sign](#wst) |
| Independent      | Difference in Variance | [Tukey-Siegel](#tst) |
| More than two groups |- | See [ANOVA](#anova)

### Wilcoxon Rank-Sum Test {#mwu}
...
#### Test Concept
To perform the Wilcoxon rank-sum test we must rank the response from lowest to highest, over both samples; both samples are pooled together and then the data is [ranked](#rank). 

The Wilcoxon rank-sum test relies on the intuition that if the two groups have the same distribution then they should have, on average, the same amount of high and low [ranked](#rank) variables. 

The test checks to see if one group has an abnormally large amount of high ranked variables. 

#### Assumptions

- variable of interest is [continuous/ordinal](#gloss),
- data has only two groups, and
...
- within-subject/within-pair observations can be dependent; and
...
- variable of interest is continuous/ordinal;
- data has only two groups;
- all responses are independent; and
...
#We expect to reject here
PMCMRplus::siegelTukeyTest(x=s1-median(s1),
                           y=s2-median(s2))
...
- response/dependent variable is [continuous/ordinal](#gloss);
- data has more than 2 groups (see [here](mwu) for two group methods.); and
- all responses are independent. 
...
- data has more than two treatments/groups/time periods (see [here](#wst) for two periods only);
- subjects are independent; 
- within subject measurements can be dependent; and
...
`friedman_test` has a data argument where you specify your data frame that contains your treatment variable, subject variable and your response variable. 
...
For small samples, see Note 1. in the [Kruskal Wallis ANOVA section](#anova). 
...
Suppose we want to compute all pairwise differences, to see between which groups there was differences. To compute all pairwise comparisons, use the `frdAllPairsExactTest` in the **PMCMRplus** package. 
...
Before proceeding, if the reader is not familiar with the term sampling distribution, then it is useful to read the section [sampling distributions](#sd). 
...
estimator=function(orig_data,ind){mean(orig_data[ind])}
...
estimator=function(orig_data,ind){model_b=lm(Hwt~Bwt,orig_data[ind,]); coef(model_b)}
...
Put simply, if your goal is predicting something, you may want to build a random forest. How random forests work is somewhat complicated, so we omit the details. The [additional resources section](#arrf) contains some educational material on random forests. 
...
```{r Random Forest Iris Example,cache=TRUE}
#iris data
...
```{r Random Forest Regression Example,cache=TRUE}
...
### Additional Resources for Random Forests {#arrf}
