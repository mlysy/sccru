...
```
This data set contains information of students taking a economics class in college. The details can be found in the reference manual of the package. 
...
The data set contains some missing data. In this chapter, we will only analyze the observations that are complete. Therefore, we will discard the data points with missing field and gather them in a new data set `econ`.
...
We can now check the structure of the new dat set `econ`. Notice how it is different from the original `econmath` data set. The categorical variables are now treated as categorical (`Factor`) in R.  
...
Consider the case where we are interested to know how an indepedent variable $x$ is associated $y$. Suppose we have a *random* sample of size $n$ \{$(x_i, y_i)$: $i=1,\ldots, n$\} following the model
$$
...
[Standard error](https://en.wikipedia.org/wiki/Standard_error) is the square root of the variance which also gives us information about the variabilities of the estimated parameters. Hence, it is usually reported with the estimated parameters. In R, the standard errors are included in the the summary of the simple linear model with the function `summary()`. For example, in model `slm`, the standard error of $\hat{\beta}_1$ is 0.7051. 
...
It is not always possible to assess the independence assumption in practice. If data are serially correlated (e.g., measurements over time, say), we may be able to identify violation of the independence assumption by plotting residuals against their natural ordering. If there is no serial correlation, we should see a horizontal band around 0 with no specific pattern.
...
One reason for the problem is that there maybe more variables that can explain `score` instead of only `colgpa`. We can try to solve this by fitting a [multiple linear regression](#linreg-mlm) model.
...
```
So we can transform `score` to `score^2` so that we have a model that satisfy the homoscedasticity assumption. 
...
Similar to the [simple linear regression models](#linreg-slm), the values of the indepedent variable in the data set $x = (x_{i1}, \ldots, x_{ip})$ for $i = 1, ..., n$ are fixed and known while the model parameters $\beta_0, \beta_1, ..., \beta_p$ and $\sigma$ are fixed but unknown. The multiple linear regression model also assumes the [LINE assumptions](#linreg-slm-assumption).
...
The dataset `econ` has 15 independent variables, hence our linear regression models can contain any combination of these variables or their interactions. So which model we should choose? A good model should
...
We can see the same problem from Section [3.2.5.4](#linreg-powertransform) that the varibility of the residuals are not constant with respect the the fitted values (heteroskedasticity). One solution is to use power transformation as in Section [3.2.5.4](#linreg-powertransform) to try solve this problem. 

Further discussion about the situation where each of the LINE [assumptions](#linreg-slm-assumption) are invalid can be found in Section [3.2.5](#linreg-slm-modelcheck)
...
<!-- Obsrvations with high Cook's distance will have a high influence on the output of the regression model, which can be shown in the first plot. Observations with high leverage will be far away from other observations. --> 
