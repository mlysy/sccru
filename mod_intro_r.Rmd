...
In the real world, data are recorded in different formats such as Excel spreadsheet (`xls`), Comma Separated Values (`csv`) or Text (`txt`). Each row of a data files is an observation while each column is a variable or a feature. 

Data are imported into the R Environment using functions such as `read.csv()` and `read.table()`. Imported data are stored as a data frame object. In this section, we will look at two data sets: `r embed_data_file("data/caliRain.csv", text = "caliRain.csv")` and `r embed_data_file("data/drinks.csv", text = "drinks.csv")`. 
...
Suppose we are interested to analyze `DISTANCE` in metres ($1 \tx{ ft} = 0.3048 \tx{ m}$). We can make the conversion and save it as a new column called `DISTANCE_M` in the data set. 
...
str(rain_df)
```
Here, the variable `SHADOW` is recorded as a numeric value. This is not an accurate depiction of the data set. To ensure the analysis can be done properly, we need to convert the values in `SHADOW` into categorical values in the data set. To do so, we use the function `factor()`.
...
```
Here, the numerical values 1 and 2 are set to “Westward” and “Leeward”, respectively. When we check the structure of the data set after the transformation, the variable `SHADOW` is now stored as a categorical variable (or factor).
...
There are a wide variety of plots that can be created using R, but we will focus on some of our favourites:
...
In order to create bar graphs, we need to summarize data using table.
...
A cross-tabulation table (or contingency table) can also be done. Suppose we are intested to create cross-tab for the variables `hasMilk` and `temp` in the `drinks_df`, we can do the following:
...
Notice that there is no space in between the bars like in the bar graph. This is because the graph is for continuous variable instead of categorical variable.
...
We recommend typing the codes in the Source Editor and then execute the codes. This way, there is a copy of what was done for future references.
...
For those who want to save the Environment for future use, we recommend to save the Environment using the function `save.image()` rather than using the default files `.RData`. If we only want to save certain values, we can use the function `save()` and then load the saved Environment later using the `load()` function. 
...
To install a library, say `r cran_link("ggplot2")`, include the following code at the beginning of the file:
```{r rintro-basic-require, echo=TRUE, eval=FALSE, comment=NA}
require("ggplot2")
```
This line of code install the package if it is not installed already, and load the package to the Environment. This can be achieved by using two separate functions `install.packages()` and `library()` in R, respectively.
...
R has an extensive documentation and resources for help. To read the documentation of a function, add a question mark before the name of a function. 
