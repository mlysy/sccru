...
- Name all code chunks according to the above guideline about cross references. For code chunk that takes long time to run, use `cache=TRUE` to prevent the chunk to be executed again unless the code in the chunk has been altered. More details can be found [here](https://bookdown.org/yihui/rmarkdown-cookbook/cache.html).
...
- For math formula, if they are in a sentence, please end them with suitable punctuation. If the formula is a derivation with multiple steps, only put punctuation at the final step. Do not use colons before writing a math formula, unless they are systematic components of a list.  
