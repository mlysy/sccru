...
- To make cross-referencing consistent in **bookdown**, for all labels, including code chunks, figures, tables, sections, equations, etc., use the prefix of the corresponding file as the prefix of any label. So please choose and use a module-specific prefix for each cross-reference. For example, use label `#formatguide` for this section. Refer to `prefixes.txt` for the list of the used prefixes and add yours there. Try to use the prefix that is distinctive to other words in the text such that you can find and replace all without mistakes.

- Give names to all code chunks.  It's best for these to be only letters and numbers -- no dashes, underscores, etc.^[Dashes etc. are technically allowed, but won't work for cross-referencing Figures and Tables as explained [here](https://bookdown.org/yihui/bookdown/figures.html).  So, it's best to get in the habit of avoiding non-alphanumeric characters entirely.].
