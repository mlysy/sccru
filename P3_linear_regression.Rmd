...
Based on the information from this data set, we want to answer the question: "What factors significantly affect a student’s score in a college economics course?". To do this, we will try to find how the variable `score`, i.e., the final score in an economics course measured as a percentage, can be "explained" by other variables. Linear regression is a helpful statistical model to answer this question. 
...
- Dependent/Response/Explained/Predicted Variable: This is the variable that we want to study, usually denoted as $y$ in linear regression models. In our case, the dependent variable is `score`. For a normal linear regression model, the dependent variable needs to be continuous.

- Independent/Control/Explanatory/Predictor Variables: They are factors which may influence the dependent variable, denoted as $X$ in linear models. These variables can be of different data types, continuous or discrete. 
...
- Discrete data type only takes values over a finite set of values (levels), while continuous data type has infinite possible values over a continuous range. In the data set `econ`, there are variables that should be treated as discrete, such as `male` (gender of the student, only takes in 2 values, 0 for female and 1 for male), `mathscr` (math quize score, only takes in 11 values from 0 to 1). However, R is treating all these variables as continuous. In fact, we can see how R define each variable in the data set using the function `str()`.
    ```{r}
str(econ) # structure of the data set
    ```
    To convert a variable into discrete data type in R, we use function `factor()`.
    
    - Binary variables are discrete variables that takes in only 2 values, 1 or 0. In the data set `econ`, we have `male` (=1 if male), `econhs` (=1 if taken economics), `calculus` (=1 if taken calculus), `fathcoll` (=1 if father has BA), and `mothcoll` (=1 if mother has BA).
        ```{r}
...
We can now check the structure of the new dat set `econ`. Notice how it is different from the origial `econmath` data set. The discrete variables are now treated as discrete (`Factor`) in R.  
...
- The values of the indepedent variable $x$ in the data set $x = (x_1, \ldots, x_n)$ are fixed and known.

- The model parameters $\beta_0, \beta_1, \sigma$ are fixed but unknown.

- Given the value $x_i$, the [expectation](https://en.wikipedia.org/wiki/Expected_value) of the response $y_i$ is a linear function: 
    $$
\E(y_i|x_i) = \beta_0 + \beta_1 x_i
    $$

- The errors $\epsilon_i = y_i - \beta_0 - \beta_1 x_i$ are [independently and identically distrbuted](https://en.wikipedia.org/wiki/Independent_and_identically_distributed_random_variables) with [normal distribution](https://en.wikipedia.org/wiki/Normal_distribution), i.e. $\epsilon_i \overset{iid}{\sim} \N(0, \sigma^2)$. This means, besdies the part that can be explained by $x$, the remaining part of the response $y$, which is represented by the error $\epsilon$, is randomly normal with mean zero and constant variance.
...
We can interpret this result as "1 point increase in College GPA will lead to an average of 14.32 points increase in the course grade".

###Hypothesis Testing

Even though we have obtained a value for $\beta_1$, it is just an estimate that depends on the data set that we have. If we want to make a more general conclusion that "college GPA affects the economics final score", we need to do [hypothesis testing](https://en.wikipedia.org/wiki/Statistical_hypothesis_testing).

We first pose the [null hypothesis](https://en.wikipedia.org/wiki/Exclusion_of_the_null_hypothesis) that there is no effect of `colgpa`, i.e., $H_0 : \beta_1 = 0$.
...
summary(slm)
```
We can see that, the $p$-value for $\hat{\beta}_1$ is $< 2e-16$, which is less than $\alpha = 0.05$, so we declare that the effect of `colgpa` is significant, or, we reject the null hypothesis that there is no effect of `colgpa` at $5\%$ significance level.

###Confidence Interval {#p3-confint1}

The confidence interval of $\beta_1$ is the interval that the true value of $\beta_1$ lie in with a specified percentage of chance. 

The ($1-\alpha$)100\% confidence interval for $\beta_1$, i.e., the interval that the true value of $\beta_1$ lie in with ($1-\alpha$)100\% chance, is given by
$$
...
### Simple Linear Regression, ANOVA, and Two-Sample $t$ Test

Analysis of variance ([ANOVA](https://en.wikipedia.org/wiki/Analysis_of_variance)) is a collection of statistical models and their associated procedures (such as "variation" among and between groups) used to analyze the differences among group means.
...
or run a $t$-test of the scores between students who took economics class in high school and students who did not
```{r}
...
We can see that the $p$-values for of these procedures are all equal at 0.326. 
...
Hence in the model `mln1`, keeping `hsgpa` fixed, one unit increase in `colgpa` leads to an average increase of 12.6668 in `score`. Since the $p$-value for `colgpa` is less than $\alpha = 0.05$, we declare that `colgpa` is significant at 5\% level. The confidence intervals can be obtained in the same fashion as in Section [3.2.4](#p3-confint1).
...
Interaction can be introduced into the multiple regression model between any type of covaraites, i.e. continuous and continuous, continuous and discrete, discrete and discrete. For example, if we only have two covaraite: `colgpa` (continuous) and `calculus` (binary). We may fit a model with `calculus` as an additive main effects:
$$
...
We can check homoscedasticity and normality with the same command as in Section [3.2.5.4](#p3-powertransform). 

```{r}
plot(Mstep, which = c(1, 2), ask = FALSE)
```

We can see the same problem from Section [3.2.5.4](#p3-powertransform) that the varibility of the residuals are not constant with respect the the fitted values (heteroskedasticity). We can proceed in the same fashion using power transformation as in Section [3.2.5.4](#p3-powertransform) to try solve this problem. 
